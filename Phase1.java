...
public class Phase1 {
    ...
    void showSecondaryMenu() {
        ...
        try(Scanner scanner = new Scanner(System.in))
        {
            ...
            switch (option){
                case 'a' : {
                    System.out.print("↳ Adding a record...Please Enter a record Name : ");
                    String filename = scanner.next().trim().toLowerCase();
                    addFile(filename);
                    break;
                }
                case 'b' : {
                    System.out.print("↳ Deleting a record...Please Enter a record Name : ");
                    String filename = scanner.next().trim();
                    deleteFile(filename);
                    break;
                }
                case 'c' : {
                    System.out.print("↳ Searching a record...Please Enter a record Name : ");
                    String filename = scanner.next().trim();
                    searchFile(filename);
                    break;
                }
    ...
    void showFiles() {
        if (folder_name.list().length==0)
            ...
            System.out.println("The records in "+ folder_name +" are :");
            Arrays.sort(list);
            for (String str:list) {
    ...
    void addFile(String filename) throws IOException {
        ...
        for (String file: list) {
            if (filename.equalsIgnoreCase(file)) {
                System.out.println("record " + filename + " already exists at " + folder_name);
                return;
            }
        }
        filepath.createNewFile();
        System.out.println("record "+filename+" added to "+ folder_name);
    }
    ...
    void deleteFile(String filename) {
        ...
        for (String file: list) {
            if (filename.equals(file) && filepath.delete()) {
                System.out.println("record " + filename + " deleted from " + folder_name);
                return;
            }
        }
    ...
    void searchFile(String filename) {
        ...
        for (String file: list) {
            if (filename.equals(file)) {
                System.out.println("FOUND : record " + filename + " exists at " + folder_name);
                return;
            }
        }
        System.out.println("record NOT found (FNF)");
    }
